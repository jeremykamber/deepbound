  ...
  return (
    <div className={`flex gap-4 md:gap-8 transition-all duration-300 ${isActive || isDone ? 'opacity-100 translate-x-0' : 'opacity-10 -translate-x-1'}`}>
      ...
      <div className="flex-1">
        ...
        {isActive && screenshot && (
          <div className="mt-8 rounded-lg overflow-hidden border border-white/10 shadow-3xl aspect-video bg-black relative group/screenshot">
        ...
        {isActive && (step.id === 'THINKING' || step.id === 'BRAINSTORMING_PERSONAS' || step.id === 'GENERATING_BACKSTORIES') && streamingText && (
          <div key={streamingText.length > 0 ? 'active' : 'inactive'} className="mt-6 p-5 rounded-lg bg-white/[0.02] border border-white/10 font-mono text-[10px] text-primary/60 overflow-hidden break-words max-h-[250px] overflow-y-auto custom-scrollbar shadow-inner relative group/stream animate-in fade-in slide-in-from-bottom-1 duration-200">
